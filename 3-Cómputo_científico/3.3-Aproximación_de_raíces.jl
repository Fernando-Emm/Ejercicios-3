...
# ╔═╡ 91f22dc1-1a42-40f1-9da9-831ff9694e5c
...
function newton(f,f′,x0,N)                               #Comenzamos definiendo una función "Newton" y sus variables.
	for z in N                                               #Establecemos un rango que vaya de z en N

	x=x0-(f/f′)
		println("Raíz: ",x, " con ", z , " iteraciones")     #Implementamos el método de Newton 
	end
end
...
# Tu código (comentado) va aquí :D
	...
	function newtonRecursiva(f1,df,x0,n0)                            #Definimos la función newtonRecursiva y sus variables
		if n0 >= 0                                                   #Delimitamos n0 a valores mayores o iguales a cero
		newtonRecursiva= x0-(f1/df)                                  #implementa el méodo de Newton   
	                                                                
	end
	end


# ╔═╡ b9dde07c-2340-4865-b493-ff5133fc009a
newtonRecursiva(1.5,5,0.1,50)
...
# Tu código (comentado) va aquí :D
	...
	function newtonDistancia(F,F′,X0,epsilon)                                #Definimos una función newtonDistancia y sus variables

		if epsilon<=0                                                       #si epsilon es 0 o negativo
		println("No mi ciela tu epsilon es menor o igual a 0 y soporta")     #imprime un mensaje de error 
		else
...
# ╔═╡ 389b6338-e66f-497c-9868-c3764245c47d
newtonDistancia(1,2,0.1,-1)

# ╔═╡ 5d6ebe4d-1323-4d45-8c4c-3832522081b0
newtonDistancia(1,2,0.1,0.2)
...
derivadaNumérica(F1,X,h)= (F1(X+h) - F1(X))/h               #Definimos la función y sus variables



# ╔═╡ 04801014-a9af-40d1-a189-e3f720723ba0
derivadaNumérica(cos,0,1)

# ╔═╡ 751183c5-f789-4395-8359-efc102f3f1ec
...
newtonDN(F_1,X_0,H)=X_0 - F_1(X_0)/derivadaNumérica(F_1, X_0, H)    #implementamos la función y sus variables

# ╔═╡ de865c04-04fc-4719-85c2-6a257f8edc3d
newtonDN(cos,2,0.1)
...
# ╔═╡ d1ad1aea-2bce-4e95-8fe7-5060ce8a4477
...
begin
	...
	f(x) = cos(x)                                          #Obtendrémos las raíces de Coseno                    
	
	df(x) = -sin(x)                                        #Definimos su derivada
	
	function newton(f, df, x0, tol=1e-7, max_iter=100)     #implementamos el método de newton definido anteriormente
	    x = x0                                             #Damos una condición inicial
	    for _ in 1:max_iter                                #definimos un parámetro 
	        x_old = x
	        fx = f(x)
	        dfx = df(x)
	        
	        if abs(dfx) < tol                              #Evitamos  la división entre cero
	            println("Derivada cercana a cero!")
	            return x
	        end
	        
	        x = x_old - fx/dfx                             #Definimos la convergencia
	      if abs(x - x_old) < tol
	            return x
	        end
	    end
	...
	function graficar(f, raices, limites=(-2*pi, 2*pi))     #Grafica la función y sus raíces
	    x = range(limites[1], limites[2], length = 1000)
	    y = f.(x)
	
	    plot(x, y, label="cos(x)", linewidth=3)
	    scatter!(raices, f.(raices), label="Raices", color="red")
	    hline!([0], color="black", label="", linewidth=2)
	end
	
	
	x_iniciales = -2*pi:pi/2:2*pi                              #Establecemos las conjeturas iniciales
	raices = []
	
	for x0 in x_iniciales
	    raiz = newton(f, df, x0)
	    push!(raices, raiz)
	end
...
# ╔═╡ 15b9888c-7e64-4108-bb81-4b735bd0a48b
newton(0.5,2.1,0.1,200)
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "2e58213e7804ee0e4762335c9769da998dced96b"
